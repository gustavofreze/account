...
(
    ...
    payload           JSON         NOT NULL COMMENT '[PII] The recorded fact serialized as a JSON object in snake_case, which carries the holder document number when the fact is an account opening (e.g., {"amount": -50.00, "transaction_id": "0190d09e-a7a8-7e89-b48c-09fff0f0f0f0"}).',
    revision          INT          NOT NULL COMMENT '[NONE] The schema revision of the serialized payload (e.g., 1).',
    event_type        VARCHAR(255) NOT NULL COMMENT '[NONE] The kind of fact the event records in CamelCase (e.g., AccountOpened, AccountCredited, AccountDebited, AccountWithdrawn).',
    ...
    aggregate_type    VARCHAR(255) NOT NULL COMMENT '[NONE] The kind of aggregate that recorded the event in CamelCase (e.g., Account).',
    ...
    COMMENT = '[PII] Table used to persist outbox event records.';
