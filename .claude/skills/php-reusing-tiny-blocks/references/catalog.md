...
| `tiny-blocks/http`                 | 7.0.0          | Implements PSR-7, PSR-15, PSR-17 and PSR-18 HTTP primitives for PHP, with a fluent response builder, cookies, cache control, and a PSR-18 client facade.      |
...
| `tiny-blocks/http-error-handler`   | 1.3.0          | PSR-15 error handler that maps thrown exceptions to structured JSON error responses with optional logging.                                                    |
...
| `tiny-blocks/http-query`           | 2.4.0          | Typed, framework-independent toolkit for HTTP collection queries (RSQL filtering, sorting, and offset and cursor pagination) that never touches a data store. |
...
| `tiny-blocks/logger`               | 3.0.0          | Emits PSR-3 structured logs for PHP, with correlation tracking, a severity threshold, and configurable redaction.                                             |
...
| `tiny-blocks/time`                 | 2.6.0          | Models time as immutable value objects for PHP: instants, durations, periods, timezones, and time-of-day, all UTC-normalized.                                 |
